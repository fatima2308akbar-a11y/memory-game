 ...
 * board -- then the next layer's grid loads in. Clear both layers before
 * the 45-second timer runs out to win; run out of time first and it's
 * game over, with a Play Again option.
 *
...
(function () {
  ...
  var LAYERS = [
    ...
    [
      ...
      { id: "js2", name: "JavaScript", logo: "logo-javascript.svg" },
      { id: "react2", name: "React", logo: "logo-react.svg" }
    ]
  ];

  var LOSS_LINES = [
    "Your spidey-sense went quiet right when you needed it. The web slipped through your fingers -- swing back and try again.",
    "Time's up, wall-crawler. Green Goblin's still loose and this web's still tangled. One more shot?",
    "Even Spider-Man misses a web-shot now and then. Reset your sense and swing again."
  ];
  ...
  var timerValueEl = document.getElementById("mm-timer-value");
  var timerChipEl = timerValueEl ? timerValueEl.closest(".mm-status-chip") : null;
  ...
  function updateTimerDisplay() {
    ...
    if (timerChipEl) {
      timerChipEl.classList.toggle("mm-timer-danger", secondsLeft <= 10);
    }
  }
    ...
    endOverlayEl.innerHTML =
      '<div class="mm-end-card">' +
        ...
        '<button type="button" class="btn-complete mm-end-btn" id="mm-restart-btn">' +
          (kind === "win" ? "SWING AGAIN" : "PLAY AGAIN") +
        '</button>' +
      '</div>';
  ...
  function onLoss() {
    ...
    lockBoard = true;
    stopTimer();
    var line = LOSS_LINES[Math.floor(Math.random() * LOSS_LINES.length)];
    showEndOverlay("loss", "SENSE... LOST.", line);
  }
  ...
  function onWin() {
    ...
    showEndOverlay(
      "win",
      "WEB SECURED!",
      "Both layers mapped with " + formatTime(secondsLeft) + " left on the clock. Queens sleeps easy tonight, wall-crawler."
    );
  }
